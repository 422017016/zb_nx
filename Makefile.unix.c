...
-include $(TOPDIR)/.config //包含 Nuttx/.config 内容为构建nuttx的相关的所有配置
include $(TOPDIR)/tools/Config.mk //进入工具配置
-include $(TOPDIR)/Make.defs //进入Make.defs
...
ifeq ($(DIRLINK),) //windows下的，linux下不考虑
...
CONFIG_APPS_DIR = ../apps //stm32也选择的是默认的
endif
